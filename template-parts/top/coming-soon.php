<?php $pageName="page-coming-soon";?>
  <section class="<?php echo $pageName;?>">
    <div class="<?php echo $pageName;?>__inner">
      <div class="<?php echo $pageName;?>__left">
      <img src="<?php echo get_stylesheet_directory_uri(); ?>/img/page/fv_comingsoon.jpg" class="object_fit pctabmid-only">
      <img src="<?php echo get_stylesheet_directory_uri(); ?>/img/page/fv_comingsoon-sp.jpg" class="object_fit sp-only">
        <div class="<?php echo $pageName;?>__left-ttl font-en">Coming Soon</div>
        <ul class="<?php echo $pageName;?>__left-list font-en">
          <li class="<?php echo $pageName;?>__left-item">5 Camp</li>
          <li class="<?php echo $pageName;?>__left-item">4 Sauna</li>
          <li class="<?php echo $pageName;?>__left-item">3 Sound</li>
          <li class="<?php echo $pageName;?>__left-item">2 Water</li>
          <li class="<?php echo $pageName;?>__left-item">1 Wind</li>
          <li class="<?php echo $pageName;?>__left-item">Sessi0n</li>
        </ul>
      </div>
      <div class="<?php echo $pageName;?>__right">
        <div class="<?php echo $pageName;?>__right-bg"></div>
        <div class="<?php echo $pageName;?>__right-inner">
          <h1 class="<?php echo $pageName;?>__right-logo"><img src="<?php echo get_stylesheet_directory_uri(); ?>/img/common/logo_locolo.svg" class="object_fit"></h1>
          <span class="<?php echo $pageName;?>__right-ttl">-ロコロ・ヴィレッジ-</span>
          <div class="<?php echo $pageName;?>__right-open">淡路島にオープン予定</div>
            <p class="<?php echo $pageName;?>__right-txt">
              <span class="<?php echo $pageName;?>__right-txt-item first">
                <span class="<?php echo $pageName;?>__right-txt-item third">淡路島の大自然の中 風や音を感じ</span><br class="mid-only">
                <span>"何もしないをする"</span>
              </span><br class="pc-only">
              <span class="<?php echo $pageName;?>__right-txt-item first">
                <span class="<?php echo $pageName;?>__right-txt-item second">キャンプやサウナ、BBQ、焚き火で</span><br class="tabsp-only">
                <span>アナログな感覚を取り戻して</span>
              </span><br>
              あなたも"0 / ゼロ"に戻ってみませんか？
            </p>
          <ul class="<?php echo $pageName;?>__right-list">
            <li class="<?php echo $pageName;?>__right-item">
              <a href="https://www.youtube.com/channel/UCTOp6AMiEcsWlXDrbxabK1w" target="_blank"><img src="<?php echo get_stylesheet_directory_uri(); ?>/img/common/icn_youtube.svg" class="object_fit"></a>
            </li>
            <li class="<?php echo $pageName;?>__right-item">
              <a href="https://twitter.com/LOCOLOVILLAGE" target="_blank"><img src="<?php echo get_stylesheet_directory_uri(); ?>/img/common/icn_twitter.svg" class="object_fit"></a>
            </li>
            <li class="<?php echo $pageName;?>__right-item">
              <a href="https://www.instagram.com/locolovillage/" target="_blank"><img src="<?php echo get_stylesheet_directory_uri(); ?>/img/common/icn_instagram.svg" class="object_fit"></a>
            </li>
          </ul>
        </div>
      </div>
    </div>
    <div class="<?php echo $pageName;?>__stamp"><img src="<?php echo get_stylesheet_directory_uri(); ?>/img/top/icn_stamp.svg" class="object_fit"></div>
  </section>